...
// NOTA: lógica duplicada de src/lib/leads-unified.ts e inlinada aqui de propósito —
// o bundle serverless da Vercel não resolve import relativo fora de /api em ESM.
type LeadSourceKey = 'framer' | 'rd_pipedrive' | 'webinar';
...
interface UnifiedLead {
  ...
  created_at: string; is_indicacao: boolean; utm_source: string | null; utm_medium: string | null; utm_campaign: string | null;
...
interface UnifiedFilters {
  ...
  problemOnly?: boolean; dupOnly?: boolean; search?: string; from?: string; to?: string; limit?: number;
}
const U_REQUIRED_CORE = ['nome', 'email', 'telefone'] as const;
...
function uNormalize(source: LeadSourceKey, row: any): UnifiedLead {
  const common = source === 'rd_pipedrive'
    ? { nome: row.lead_nome ?? null, email: row.lead_email ?? null, telefone: row.lead_telefone ?? null, empresa: row.lead_empresa ?? null, produto: row.produto_interesse ?? null }
    : { nome: row.nome ?? null, email: row.email ?? null, telefone: row.telefone ?? null, empresa: row.empresa ?? null, produto: row.produto ?? null };
  const missing = U_REQUIRED_CORE.filter((f) => uIsEmpty((common as any)[f]));
  ...
  if (source === 'rd_pipedrive') {
    routing = {
      rota_definida: row.rota_definida ?? null, rota_encontrada: row.rota_encontrada ?? null,
      destino_pipeline: row.destino_pipeline_nome ?? null, destino_stage: row.destino_stage_nome ?? null,
      destino_owner: row.destino_owner_nome ?? null, processado: row.processado ?? null, motivo_rota: row.motivo_rota ?? null,
    };
...
async function fetchUnifiedLeads(db: SupabaseClient, opts: UnifiedFilters = {}): Promise<{ data: UnifiedLead[]; total: number }> {
  ...
  await Promise.all(sources.map(async (src) => {
    let q = db.from(U_TABLE[src]).select(U_SELECT[src]).gte('criado_em', from).order('criado_em', { ascending: false }).limit(2000);
  ...
  if (opts.search) {
    ...
    filtered = filtered.filter((l) => (l.email || '').toLowerCase().includes(s) || (l.nome || '').toLowerCase().includes(s) || (l.empresa || '').toLowerCase().includes(s));
  }
  filtered.sort((a, b) => (a.created_at < b.created_at ? 1 : -1));
  const total = filtered.length;
  return { data: filtered.slice(0, opts.limit ?? 200), total };
}

async function fetchUnifiedStats(db: SupabaseClient): Promise<any> {
  const since30 = new Date(Date.now() - 30 * 86400_000).toISOString();
  const { data: leads } = await fetchUnifiedLeads(db, { from: since30, limit: 100000 });
  const now = Date.now();
  const today = new Date(); today.setHours(0, 0, 0, 0);
  const t24 = now - 24 * 3600_000, t7 = now - 7 * 86400_000, tToday = today.getTime();
  const ts = (l: UnifiedLead) => new Date(l.created_at).getTime();
  const mk = (pred: (l: UnifiedLead) => boolean) => {
...
async function jFetch(db: SupabaseClient, opts: any = {}): Promise<{ data: any[]; total: number }> {
  ...
  for (const r of (fr.data as any[]) || []) {
    ...
    j.is_indicacao = j.is_indicacao || !!r.is_indicacao; j.utm_source = jFill(j.utm_source, r.utm_source); j.utm_medium = jFill(j.utm_medium, r.utm_medium); j.utm_campaign = jFill(j.utm_campaign, r.utm_campaign);
  ...
  for (const r of (rd.data as any[]) || []) {
    const k = jKey(r.lead_email); if (!k) continue; const j = get(k); j.has_rd = true; j.rd_count++;
    j.nome = jFill(j.nome, r.lead_nome); j.email = jFill(j.email, r.lead_email); j.telefone = jFill(j.telefone, r.lead_telefone); j.empresa = jFill(j.empresa, r.lead_empresa); j.produto = jFill(j.produto, r.produto_interesse);
    j.is_indicacao = j.is_indicacao || !!r.is_indicacao; j.utm_source = jFill(j.utm_source, r.utm_source); j.utm_medium = jFill(j.utm_medium, r.utm_medium); j.utm_campaign = jFill(j.utm_campaign, r.utm_campaign);
    if (r.processado === true) j.processado = true; else if (j.processado === null) j.processado = r.processado ?? null;
    j.rota_definida = jFill(j.rota_definida, r.rota_definida); j.motivo_rota = jFill(j.motivo_rota, r.motivo_rota);
  ...
  for (const r of (wb.data as any[]) || []) {
    const k = jKey(r.email); if (!k) continue; const j = get(k); j.has_webinar = true; j.webinar_count++;
    j.nome = jFill(j.nome, r.nome); j.email = jFill(j.email, r.email); j.telefone = jFill(j.telefone, r.telefone); j.empresa = jFill(j.empresa, r.empresa); j.produto = jFill(j.produto, r.produto);
    j.created_at = j.created_at ? (j.created_at < r.criado_em ? j.created_at : r.criado_em) : r.criado_em; j.last_at = jNewer(j.last_at, r.criado_em);
    j.refs.push({ source: 'webinar', id: r.id, criado_em: r.criado_em ?? null });
  }
  const all = [...map.values()];
  const dealIds = Array.from(new Set(all.map((j) => j.deal_id).filter((d) => d !== null && d !== undefined)));
  if (dealIds.length) {
    try {
      ...
      const byDeal = new Map<string, any>(); for (const d of (data as any[]) || []) byDeal.set(String(d.deal_id), d);
...
app.get('/api/journey', async (req, res) => {
  ...
  try {
    res.json(await jFetch(db, {
      stage: q.stage || undefined, health: q.health || undefined, search: q.search || undefined,
      problemOnly: q.problem === '1' || q.problem === 'true', dupOnly: q.dup === '1' || q.dup === 'true',
      from: q.from || undefined, to: q.to || undefined, limit: q.limit ? Number(q.limit) : undefined,
    }));
  } catch (e: any) { res.status(500).json({ error: e.message }); }
});
...
// Webhook Framer = execuções do fluxo Framer · Chegou no RD = Framer com sucesso
// MQL = execuções da Passagem (iniciou) · Deal = Passagem com sucesso
